...
class ReplayBuffer(object):
	def __init__(self, state_dim, action_dim, max_size=int(1e6)):
		...
		self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")#控制使用哪块GPU
...
class Hot_Plug(object):
    ...
    def update(self, lr=0.1):
        for param_name in self.params.keys():
            ...
            if lr > 0 and self.params[param_name].grad is not None:
                cursor._parameters[path[-1]] = self.params[param_name] - lr * self.params[param_name].grad
            else:
                # Some parameters may be unused in a particular inner step; keep them unchanged.
