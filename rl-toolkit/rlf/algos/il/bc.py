...
class BehavioralCloning(BaseILAlgo):
    ...
    def full_train(self, update_iter=0):
        ...
        rutils.plot_line(
            ...
            # --- 在训练结束后添加环境评估 ---
        if not self.args.no_wb:
            print("Evaluating policy in environment...")
            eval_info = autils.evaluate_policy(
                self.policy,
                self.args,
                num_episodes=self.args.eval_episodes if hasattr(self.args, "eval_episodes") else 10,
                deterministic=True,
            )
    
            # 记录 avg_ep_found_goal（如果环境返回）
            if "avg_ep_found_goal" in eval_info:
                import wandb
                wandb.log({"avg_ep_found_goal": eval_info["avg_ep_found_goal"]})
            else:
                print("⚠️ Warning: eval_info has no avg_ep_found_goal key, check your eval_env or EvalLogger.")
    
            print("Evaluation results:", eval_info)
