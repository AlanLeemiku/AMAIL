...
def run_policy(run_settings, runner=None):
    ...
    if args.ray:
        ...
        for j in range(start_update, end_update):
            ...
            # GILD 调度与环境噪声无关：噪声只由 env 传参控制，这里保持一个稳定的外部系数
            gild_coef = 1.0
        ...
        #         x = progress * k
        #         gild_part = 1 / (1 + math.exp(-x))
        #         gild_coef = y0 * (2 * (1 - gild_part))
        #     gild_coef = max(gild_coef, 0.0)

            updater_log_vals = runner.training_iter(j, gild_coef, beginning, t)
