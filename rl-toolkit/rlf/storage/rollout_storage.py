...
# 这个函数 feed_forward_generator 主要用于在 PPO（Proximal Policy Optimization）等强化学习算法中生成小批量数据（mini-batches），用于训练策略网络。它将完整的经验回放缓冲区（replay buffer）数据拆分成小批量，并进行 随机采样，然后返回一个包含多个训练数据项的字典，用于训练策略网络。
    ...
    def sample(self, mini_batch_size=None, num_mini_batch=None):
        """
        Sample a single mini-batch of training data from the rollout storage.

        Returns:
            A dict containing:
            - state: 当前观察
            - other_state: 额外观察（非主观察）
            - reward: 奖励
            - hxs: 隐状态（如果使用）
            - action: 行动
            - value: 价值估计
