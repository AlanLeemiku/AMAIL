...
def eval_print(
    ...
    # Log per-evaluation metrics with a prefix (e.g. eval_train_ep_found_goal)
    log.log_vals(
        {"eval_%s_%s" % (mode, k): np.mean(v) for k, v in eval_info.items()},
        total_num_steps,
    )
    
    # Also log a top-level average found-goal metric so algorithms that don't
    # collect episode-step stats in the main loop (e.g. bc or dp) still have
    # a consistent field `avg_ep_found_goal` in W&B summaries/reports.
    if "ep_found_goal" in eval_info:
        try:
            log.log_vals({"avg_ep_found_goal": eval_info["ep_found_goal"]}, total_num_steps)
        except Exception:
            # Be defensive: don't break evaluation if logging fails
            pass
