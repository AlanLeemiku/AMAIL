...
class Runner:
    ...
    def __init__(
        self, envs, storage, policy, log, env_interface, checkpointer, args, updater, meta_rollout_bc, meta_rollout_gild
    ):
        self.envs = envs
        ...
        self.gild_coef = None
        # self.meta_rollout_bc = meta_rollout_bc
        # self.meta_rollout_gild = meta_rollout_gild
    ...
    def training_iter(self, update_iter: int, gild_coef, beginning=False, t=1) -> Dict[str, Any]:
        self.log.start_interval_log()
        self.updater.pre_update(update_iter)
        self.gild_coef = gild_coef
        for step in self.updater.get_steps_generator(update_iter):
            ...
            # self.replaybuffer.push(obs, action, next_obs, reward, done)
       
        action_shape=self.policy.action_space.shape           
        # obs_shape=obs.shape
        updater_log_vals = self.updater.update(self.storage, self.gild_coef, self.args, beginning, t)
        # updater_log_vals = self.updater.update(self.storage,self.meta_rollout_bc,self.meta_rollout_gild,self.gild_coef, self.args, beginning, t)

        self.storage.after_update()
        # self.meta_rollout_bc.after_update()
        # self.meta_rollout_gild.after_update()
