...
from rlf.algos.on_policy.ppo import PPO_GILD
...
class Discriminator(nn.Module):
    ...
    def diffusion_loss(self, label, sa_pair, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, n_steps):
        ...
        if self.args.sample_strategy == "constant":
            ...
            half = batch_size // 2
            t1 = torch.randint(0, n_steps, size=(half,)).to(self.args.device)
            t2 = n_steps - 1 - t1
            t = torch.cat([t1, t2], dim=0)
            # 如果 batch_size 是奇数，补一个
            if t.shape[0] < batch_size:
                extra = torch.randint(0, n_steps, size=(1,)).to(self.args.device)
...
class DRAIL(NestedAlgo):
    def __init__(self, agent_updater=PPO_GILD(), get_discrim=None):
...
class DRAILDiscrim(BaseIRLAlgo):
    ...
    def _create_discrim(self):      #引入扩散模型模拟判别器
...
# 这个函数 _compute_discrim_loss 主要用于计算判别器（Discriminator）的损失，它在模仿学习（Imitation Learning）或逆强化学习（Inverse Reinforcement Learning）中很常见，通常用于区分专家（Expert）和智能体（Agent）的行为，以进行对抗性训练（如 GAIL：Generative Adversarial Imitation Learning）。
    ...
    def plot_reward_map(self, i):
        ...
        with torch.no_grad():
            ...
            if self.args.reward_type == 'airl':
                reward = (s + eps).log() - (1 - s + eps).log()#扩散奖励
    ...
    def _update_reward_func(self, storage, gradient_clip=False, t=1):
        ...
        for epoch_i in range(self.args.n_drail_epochs):
            for expert_batch, agent_batch in zip(expert_sampler, agent_sampler):
                ...
                discrim_loss = expert_loss + agent_loss  #判别器优化目标函数
    ...
    def _compute_discrim_reward(self, storage, step, add_info):
        ...
        eps = 1e-20
        if self.args.reward_type == 'airl':
            reward = (s + eps).log() - (1 - s + eps).log()#######计算用于更新策略网络的奖励
        elif self.args.reward_type == 'gail':
            reward = (s + eps).log()
        elif self.args.reward_type == 'raw':
            reward = s
